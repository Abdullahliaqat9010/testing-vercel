  ...
  useEffect(() => {
    if (mainProperty.lat && mainProperty.lng) {
      const arrDist = agentsList.map((agency, index) => {
          return {
            agencyIndex: index,
            dist: +calcCrow(+mainProperty.lat, +mainProperty.lng, agency.location.lat, agency.location.lng).toFixed(1),
          };
        },
      );
  ...
  return (
    <div className='find-agent-block'>
      ...
      <p>{ t('desc.we-found') } { agentsList.length } { t('desc.agents-near-you') }</p>
      {
        agencyList.map(
          (agency, index) =>
            <Agency nearest={agencyList[0]?.id} agency={ agency } key={ index }/>,
        )
      }
      {
